...
const API_BASE_URL = import.meta.env.VITE_API_UR || 'http://localhost:8000/api';
...
export const diagramsAPI = {
  ...
  // Save complete diagram with elements and relationships
  saveComplete: async (diagramId: string, diagramData: any) => {
    const response = await api.post(`/diagrams/${diagramId}/save-complete/`, diagramData);
    return response.data;
  },
  // Load complete diagram with elements and relationships
  loadComplete: async (diagramId: string) => {
    const response = await api.get(`/diagrams/${diagramId}/load-complete/`);
    return response.data;
  },
};
...
export const diagramElementsAPI = {
  ...
  list: async (diagramId?: string) => {
    const url = diagramId ? `/diagram-elements/?diagram_id=${diagramId}` : '/diagram-elements/';
  ...
  update: async (id: string, data: any) => {
    const response = await api.put(`/diagram-elements/${id}/`, data);
    return response.data;
  },
  // Delete element
  delete: async (id: string) => {
    return api.delete(`/diagram-elements/${id}/`);
  },
  // Get element by ID
  get: async (id: string) => {
    const response = await api.get(`/diagram-elements/${id}/`);
    return response.data;
  },
  // Update position
...
export const relationshipsAPI = {
  ...
  getRelationships: async (diagramId: string): Promise<DiagramRelationship[]> => {
    try {
      const response: AxiosResponse<DiagramRelationship[]> = await api.get(`/diagrams/${diagramId}/`); 
