...
const Sidebar: React.FC<SidebarProps> = ({ onItemClick, open = true, onToggle }) => {
  ...
  // Detectar si estamos en el editor y extraer diagramId
  const editorMatch = location.pathname.match(/^\/editor\/([^/]+)$/);
  const diagramId = editorMatch ? editorMatch[1] : null;

  const toolsNavItems: NavItem[] = [
    {
      text: 'Generación de Código',
      icon: <Code />,
      path: diagramId ? `/code-generation/${diagramId}` : '/code-generation',
    },
    {
      text: 'Exportar PDF',
      icon: <PictureAsPdf />,
